...
CASH_FILE = os.getenv("CASH_FILE", "cash_balance.txt")

# Cash management defaults
INITIAL_CASH = float(os.getenv("INITIAL_CASH", ACCOUNT_VALUE))
...
# ═════════════════════════════════════════════════════════════════════════
def read_cash_balance(path: str = CASH_FILE) -> float:
    """Read current cash balance from file. Initialize with INITIAL_CASH if file doesn't exist."""
    if os.path.isfile(path):
        try:
            with open(path) as f:
                return float(f.read().strip())
        except (OSError, ValueError) as exc:
            logger.warning("Error reading cash balance from %s: %s", path, exc)
    logger.info("Initializing cash balance with %.2f", INITIAL_CASH)
    write_cash_balance(INITIAL_CASH, path)
    return INITIAL_CASH


def write_cash_balance(balance: float, path: str = CASH_FILE) -> None:
    """Write current cash balance to file."""
    with open(path, "w") as f:
        f.write(f"{balance:.2f}")
    logger.info("Cash balance written: %.2f", balance)


def deposit_cash(amount: float, path: str = CASH_FILE) -> float:
    """Add cash to the account. Returns new balance."""
    if amount <= 0:
        raise ValueError("Deposit amount must be positive")
    current = read_cash_balance(path)
    new_balance = current + amount
    write_cash_balance(new_balance, path)
    logger.info("Deposited %.2f, new balance: %.2f", amount, new_balance)
    return new_balance


def withdraw_cash(amount: float, path: str = CASH_FILE) -> float:
    """Withdraw cash from the account. Returns new balance.
    ...
    current = read_cash_balance(path)
    if amount > current:
        raise ValueError(f"Insufficient funds: {amount} > {current}")
    new_balance = current - amount
    write_cash_balance(new_balance, path)
    logger.info("Withdrew %.2f, new balance: %.2f", amount, new_balance)
    return new_balance
...
# ── 3 ▸ ATR position size ────────────────────────────────────────────────
def target_shares(k: KiteConnect, sym: str, total_value: float) -> int:
    """ATR-based risk parity sizing with MAX_WEIGHT cap.

    Size = (total_value × RISK_FACTOR) / ATR
    ...
    shares = (total_value * RISK_FACTOR) / atr_value
    price = float(df["close"].iloc[-1])
    cap = (total_value * MAX_WEIGHT) / price if price > 0 else 0
...
def total_account_value(k: KiteConnect, pf: dict[str, int]) -> float:
    """Total account value: cash + portfolio value."""
    return read_cash_balance() + live_value(k, pf)
...
def resize_positions(k: KiteConnect, pf: dict[str, int], bull: bool) -> None:
    ...
    total_value = total_account_value(k, pf)
    to_up, to_down = {}, {}
    for sym, qty in pf.items():
        try:
            tgt = target_shares(k, sym, total_value)
        except Exception as exc:
    ...
    # 1️⃣ sell downs first - update cash balance
    cash_gained = 0.0
    for sym, delta in to_down.items():
        price = k.quote([f"NSE:{sym}"])[f"NSE:{sym}"]["last_price"]
        safe_sell(k, sym, delta)
        pf[sym] -= delta
        if pf[sym] == 0:
            pf.pop(sym)
            sold_symbols.add(sym)
        cash_gained += delta * price

    if cash_gained > 0:
        current_cash = read_cash_balance()
        write_cash_balance(current_cash + cash_gained)
...
# ═════════════════════════════════════════════════════════════════════════
def manage_cash():
    """Interactive cash management interface for deposits/withdrawals."""
    while True:
        ...
        if choice == "1":
            try:
                ...
                new_balance = deposit_cash(amount)
...
def main() -> None:
    ...
    cash_from_exits = 0.0
    pre_exit_portfolio = portfolio.copy()
    prune_portfolio(kite, ranks, portfolio)

    # Calculate cash gained from exits
    for sym in pre_exit_portfolio:
        if sym not in portfolio:
            price = kite.quote([f"NSE:{sym}"])[f"NSE:{sym}"]["last_price"]
            cash_from_exits += pre_exit_portfolio[sym] * price

    if cash_from_exits > 0:
        current_cash = read_cash_balance()
        write_cash_balance(current_cash + cash_from_exits)

    # 7) Size parity rebalance (every even ISO week in IST)
    resize_positions(kite, portfolio, bull)

    # 8) Current cash and total account value
    cash_available = read_cash_balance()
    total_value = total_account_value(kite, portfolio)
    logger.info("Cash available: %.0f, Total account value: %.0f", cash_available, total_value)

    # 9) New buys if bull regime and cash available
    if bull and cash_available > 0:
        for r in ranks:
            ...
            if r.symbol in portfolio or r.symbol in sold_symbols:
                continue
            try:
                qty = target_shares(kite, r.symbol, total_value)
            except Exception as exc:
            ...
            cash_available -= cost
            write_cash_balance(cash_available)
    else:
...
if __name__ == "__main__":
    ...
    # Check for cash management mode
    if len(sys.argv) > 1 and sys.argv[1] == "--cash":
        manage_cash()
    else:
        main()
